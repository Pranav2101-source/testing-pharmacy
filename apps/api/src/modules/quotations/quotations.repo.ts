...
export class QuotationsRepo {
  ...
  async convertToPO(id: string, pharmacyId: string, userId: string, notes?: string) {
    return this.db.$transaction(async (tx) => {
      ...
      // PO number — same pattern as PurchasesRepo.nextPONumber
      const poCount     = await tx.purchaseOrder.count({ where: { pharmacyId } });
      const year        = new Date().getFullYear();
      const orderNumber = `PO-${year}-${String(poCount + 1).padStart(5, "0")}`;
