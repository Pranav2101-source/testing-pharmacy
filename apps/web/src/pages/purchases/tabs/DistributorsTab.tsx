...
export function DistributorsTab({ onSupplierAdded }: { onSupplierAdded: (s: FullSupplier) => void }) {
  ...
  const { data, isPending, isFetching, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      return data.data as { items: FullSupplier[]; total: number };
    },
    staleTime: 30_000,
  ...
  function handleSaved(saved: FullSupplier) {
    ...
    if (wasAdd) onSupplierAdded(saved);
  }

  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-2 px-4 py-2.5 border-b border-slate-100 bg-white flex-shrink-0">
        <div className="flex items-center border border-slate-200 rounded-lg bg-slate-50/60 overflow-hidden h-8 max-w-[280px] flex-1 transition-colors focus-within:bg-white focus-within:border-blue-300 focus-within:ring-2 focus-within:ring-blue-50">
        ...
        <span className="text-[12px] text-slate-400 font-medium ml-1">{total} distributor{total !== 1 ? "s" : ""}</span>
            ...
            {loading ? (
              ...
              <tr key={s.id} className={cn(
                ...
                <td className="px-4 py-3 text-[13px] font-bold text-slate-900 tabular-nums">
                  {s.creditLimit > 0 ? `₹${s.creditLimit.toLocaleString("en-IN")}` : <span className="text-slate-300 font-normal">—</span>}
                </td>
