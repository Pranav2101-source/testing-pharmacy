...
import { Eye, EyeOff, Lock, CheckCircle2, X, ShieldCheck, AlertCircle } from "lucide-react";
...
function getStrength(password: string): StrengthResult {
  ...
  return { score, label: map[score]?.label ?? "Too weak", color: map[score]?.color ?? "bg-red-400", checks };
}

// ─── Password field ────────────────────────────────────────────────────────────

function PasswordField({ label, placeholder, value, onChange, hint }: {
  label:       string;
  placeholder: string;
  value:       string;
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <div className={cn(
        ...
        <Lock className={cn("w-3.5 h-3.5 flex-shrink-0", focused ? "text-brand-500" : "text-slate-400")} strokeWidth={1.8} />
        <input
...
export default function ChangePasswordPage() {
  ...
  const canSubmit = current.length > 0 && strength.score >= 3 && matchOk && !sameAsOld;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Server rotated tokenVersion — all tokens are now invalid.
      // Clear client state and redirect to login after a brief confirmation delay.
      setTimeout(() => {
        clearSession();
    ...
    <div className="max-w-lg mx-auto px-6 py-8 space-y-6">
      ...
      >
        <div className="p-6 space-y-5">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className={cn(
              ...
              matchOk   ? "border-emerald-400 ring-1 ring-emerald-200" : "border-slate-200 hover:border-slate-300",
            )}>
              ...
              <AnimatePresence mode="wait">
                {matchOk   && <motion.span key="ok"   initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }}><CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" strokeWidth={2} /></motion.span>}
                {matchFail && <motion.span key="fail" initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }}><X className="w-3.5 h-3.5 text-red-400" strokeWidth={2} /></motion.span>}
              </AnimatePresence>
            </div>
            <AnimatePresence>
              {matchFail && (
        ...
        <div className="px-6 py-4 border-t border-slate-100 bg-slate-50/60 flex items-center justify-between gap-4">
          ...
          <button
            type="submit"
            disabled={!canSubmit || state === "loading" || state === "success"}
            className={cn(
          ...
          {state === "error" && errMsg && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}
              ...
              <p className="text-xs text-red-500 font-medium flex items-center gap-1.5">
                <AlertCircle className="w-3 h-3 flex-shrink-0" />{errMsg}
              </p>
            </motion.div>
          )}
        </AnimatePresence>
