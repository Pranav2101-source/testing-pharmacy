...
function stockLabel(med: MedicineSearchResult): { text: string; title?: string } | null {
  ...
  if (med.allowLooseSale && med.sellableUnits != null && med.unitsPerPack) {
    // Compact for the dropdown — the full "N packs × U = S units" breakdown stays on
    // the hover title so nothing is lost.
    return {
      text: `${med.sellableUnits} u`,
      title: `${qty} pack${qty === 1 ? "" : "s"} × ${med.unitsPerPack} = ${med.sellableUnits} sellable units`,
    };
  }
  return { text: `${qty} pk`, title: `${qty} pack${qty === 1 ? "" : "s"} in stock` };
}
...
function SkeletonResult() {
  return (
    <li className="flex items-center gap-2 px-3 py-2 border-b border-slate-50">
...
// Compact badge — tighter than the global `.pill` (which is px-2 py-0.5 text-[10px]);
// the search list is a dense POS lookup, not a card list, so every badge here is
// smaller and hugs the medicine name instead of being pushed to the far right.
const SPILL = "inline-flex items-center rounded-full px-1.5 py-px text-[9px] font-bold leading-none whitespace-nowrap";
  ...
  return (
    <>
      ...
      <div ref={containerRef} className="relative flex-1">
        ...
        <div className={cn(
          "flex items-center gap-3 px-4 py-2 rounded-none transition-all duration-150",
          focused
        ...
        <AnimatePresence>
          {open && debouncedQuery.length > 0 && (loading || results.length > 0) && (
            <motion.ul
              ...
              className="absolute z-50 left-0 top-full w-fit min-w-[460px] max-w-[680px] bg-white border border-slate-200 rounded-lg shadow-card-lg overflow-hidden max-h-[24rem] overflow-y-auto"
            >
              {loading && results.length === 0
                ...
                : results.map((med, i) => {
                    ...
                    <li
                      ...
                      className={cn(
                        "flex items-center gap-2 px-3 py-2 cursor-pointer border-b border-slate-50 last:border-0 group",
                        ...
                        // in-stock-first partition) and are still selectable — just clearly
                        // deprioritized (dimmed + desaturated) so a pharmacist can tell at a
                        // glance why it's last.
                        stock && med.inStock === false && i !== cursor && "opacity-55 saturate-50"
                      )}
                    >
                      <div className={cn(
                      ...
                      {/* Name + details — name on the primary line, generic/manufacturer/
                          strength on a compact secondary line, both truncated. NOT
                          `flex-1`: the block is only as wide as its content (capped at
                          300px, where it ellipsizes), so the stock / GST / Alt badges sit
                          right next to the name instead of being pushed away. The <ul>
                          itself is `w-fit`, so the dropdown shrink-wraps this content
                          rather than stretching to a fixed width. */}
                      <div className="min-w-0 max-w-[300px] shrink">
                        <div className="flex items-center gap-1 min-w-0">
                          <p className="text-[13px] font-semibold text-slate-800 truncate leading-tight">{med.name}</p>
                          {med.isLocal ? (
                            <span
                              title="Not yet in the shared medicine catalogue — this pharmacy's own"
                              className={cn(SPILL, "bg-amber-100 text-amber-700 uppercase tracking-wide flex-shrink-0")}
                            >
                              Local
                            </span>
                          ) : (
                          ...
                          {canClassify && !med.category && !med.isLocal && (
                            <button
                              ...
                              className="flex-shrink-0 inline-flex items-center gap-0.5 text-[9px] font-semibold text-blue-500 hover:text-blue-700 border border-dashed border-blue-200 hover:border-blue-400 rounded-full px-1 py-px transition-colors"
                            >
                      ...
                      <div className="flex items-center gap-1 flex-shrink-0">
                        {med.packSize && (
                          <span className={cn(SPILL, "bg-slate-100 text-slate-500 font-semibold")}>{med.packSize}</span>
                        )}
                        ...
                        {med.allowLooseSale && (
                          <span className={cn(SPILL, "bg-amber-100 text-amber-700")}>LOOSE OK</span>
                        )}
                        <span className={cn(SPILL, "bg-blue-100 text-blue-600")}>GST {med.gstRate}%</span>
                        {i === cursor && (
                          <span className={cn(SPILL, "bg-emerald-100 text-emerald-600")}>↵</span>
                        )}
                        ...
                        {!med.isLocal && (med.hasAlternatives || med.genericName) && (
                          <button
                            ...
                            className={cn(
                              "flex items-center gap-0.5 h-5 px-1.5 rounded-full text-[9px] font-bold transition-colors flex-shrink-0",
                              med.hasAlternatives
